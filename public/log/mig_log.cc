...
int log_impl(const char *file, const int line, const char *func, int level,
  ...
  if (priority != NO_PRIORITY) {
    if (file == NULL || line == 0 || func == NULL)
      ...
      message_len += snprintf(message, _LOG_LENGTH,
                              "[File: %s, Line: %d, Func: %s, Tid: %ld]: ",
                              file, line, func, tid);
  }
